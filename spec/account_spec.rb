...
describe Account do
  ...
  it 'expects to raise "Insufficent funds" error if withdrawl greater than balance' do
    test_account = Account.new
  ...
  it 'expects to print a statement containing all transactions in chronological order' do
    test_account = Account.new
    test_account.deposit(50)
    test_account.withdraw(20)
    test_account.deposit(500)
    expect { test_account.account_statement }.to output(
      "date || credit || debit || balance\n17/06/22 || 500.00 ||  || 530.00\n17/06/22 ||  || 20.00 || 30.00\n17/06/22 || 50.00 ||  || 50.00\n"
    ).to_stdout
  end
end
